...
SELECT
    *,
    LEAD(game_technical_rc)
        OVER (PARTITION BY player_id ORDER BY mlbam_game_date, mlbam_game_id)
    AS next_game_technical_rc,
    LEAD(game_technical_rc, 3)
        OVER (PARTITION BY player_id ORDER BY mlbam_game_date, mlbam_game_id)
    AS next_3_game_technical_rc,
    LEAD(game_technical_rc, 14)
        OVER (PARTITION BY player_id ORDER BY mlbam_game_date, mlbam_game_id)
    AS next_14_game_technical_rc,
    LAG(game_technical_rc)
        OVER (PARTITION BY player_id ORDER BY mlbam_game_date, mlbam_game_id)
    AS last_game_technical_rc,
    LAG(game_technical_rc, 3)
        OVER (PARTITION BY player_id ORDER BY mlbam_game_date, mlbam_game_id)
    AS last_3_game_technical_rc,
    LAG(game_technical_rc, 14)
        OVER (PARTITION BY player_id ORDER BY mlbam_game_date, mlbam_game_id)
    AS last_14_game_technical_rc
